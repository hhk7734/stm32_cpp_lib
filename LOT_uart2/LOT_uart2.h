...
#ifndef _LOT_UART2_H_
#define _LOT_UART2_H_
...
#define LOT_UART2_TX_BUF_SIZE 64
#define LOT_UART2_RX_BUF_SIZE 64
...
public:
     ...
     */
    inline void setup( void ) { HAL_UART_Receive_IT( &huart2, &rx_buf[rx_buf_head], 1 ); }
...
protected:
    ...
    uint8_t rx_buf[LOT_UART2_RX_BUF_SIZE];
    uint8_t tx_buf[LOT_UART2_TX_BUF_SIZE];
...
inline void LOT_uart2::tx_cplt_callback( UART_HandleTypeDef *huart )
{
    if( huart->Instance == USART2 )
    {
        if( tx_buf_head != tx_buf_tail )
        {
            ...
            tx_buf_tail = ( tx_buf_tail + 1 ) % LOT_UART2_TX_BUF_SIZE;
        }
    }
}

extern LOT_uart2 uart2;
